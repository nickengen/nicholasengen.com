document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyContactConfig = () => {
    if (emailLink && emailValue) {
      const isPlaceholder = siteConfig.email === "your@email.com";
      emailValue.textContent = isPlaceholder ? "Replace in script.js" : siteConfig.email;
      emailLink.href = isPlaceholder ? "#" : `mailto:${siteConfig.email}`;
      emailLink.setAttribute("aria-disabled", String(isPlaceholder));
    }

    if (resumeDownloadLink) {
      resumeDownloadLink.href = siteConfig.resumePath;
      resumeDownloadLink.setAttribute("download", "Nicholas-Engen-Resume.pdf");
    }

    if (resumePreviewLink) {
      resumePreviewLink.href = siteConfig.resumePath;
    }
  };
